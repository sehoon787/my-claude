#!/usr/bin/env node
// Claude Code mid-session Briefing Vault updater.
...
function updateScaffolds(payload) {
  ...
  const sessionEnd = path.join(home, '.claude', 'hooks', 'session-end.js');
  if (!runtime.exists(sessionEnd)) return;
  runNode(sessionEnd, payload, { MY_CLAUDE_SESSION_SYNC_ONLY: '1' });
}

function updateProfileIfNeeded(payload, force) {
  const home = process.env.HOME || process.env.USERPROFILE || '';
  const stopProfile = path.join(home, '.claude', 'hooks', 'stop-profile-update.js');
