#!/usr/bin/env node
// Claude Code session-end synthesis for Briefing Vault.
...
function sessionChangedFiles() {
  const stateDir = gitPath('my-claude-attribution');
  if (!stateDir) return [];
...
function appendWrapperAgentEvent(agentLogPath, payload, state) {
  ...
  const wrapper = (payload && payload.agent_type) || process.env.MY_CLAUDE_SESSION_END_AGENT_TYPE || 'wrapper';
  const wrapperId = (payload && payload.agent_id) || process.env.MY_CLAUDE_SESSION_END_AGENT_ID || 'claude-wrapper-stop';
  const event = {
...
function sessionAutoContent(today, state, changedFiles, statusLines, signalLines, enforcementReason, diffSummary) {
  ...
  return [
    ...
    `date: ${today}`,
    'type: session-auto',
    'tags: [briefing-vault, claude, auto]',
    '---',
    '',
...
function runNodeHook(scriptName, payload) {
  const scriptPath = path.join(process.env.HOME || process.env.USERPROFILE || '', '.claude', 'hooks', scriptName);
...
const syncOnly = process.env.MY_CLAUDE_SESSION_SYNC_ONLY === '1';
...
if (!syncOnly) {
  runNodeHook('stop-profile-update.js', payload || {
    agent_id: 'claude-wrapper-stop',
    agent_type: process.env.MY_CLAUDE_SESSION_END_AGENT_TYPE || 'wrapper'
  });
}
